...
SWITCH_STANDARD_CHAT_APP(mod_smpp_chat_send_multi)
{
	...
	mod_smpp_gateway_send_multi(gateway, message);
	return SWITCH_STATUS_SUCCESS;
}
...
SWITCH_STANDARD_API(mod_smpp_query_api)
{
	...
	if (!(argc = switch_separate_string(cmd_dup, '|', argv, (sizeof(argv) / sizeof(argv[0]))))) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CONSOLE, "Invalid format. Must be | separated like: gateway|source|message_id\n");
	...
	mod_smpp_gateway_send_query_sm(gateway, argv[1],argv[2]);
...
switch_status_t mod_smpp_do_config() 
{
	...
	if ( (gateways = switch_xml_child(cfg, "gateways")) != NULL) {
		for (gateway = switch_xml_child(gateways, "gateway"); gateway; gateway = gateway->next) {		
			...
			char *host = NULL, *system_id = NULL, *password = NULL, *profile = NULL, *system_type = NULL , *data_coding_recognize = NULL;
			char *mon_ts = NULL;
			int port = 0, debug = 0;
			...
			if ( (params = switch_xml_child(gateway, "params")) != NULL) {
				for (param = switch_xml_child(params, "param"); param; param = param->next) {
					...
					} else if ( ! strncmp(var, "data_coding_recognize", 21) ) {
						data_coding_recognize = (char *) switch_xml_attr_soft(param, "value");
					} else if ( ! strncmp(var, "monitoring_interval", 2) ) {
						mon_ts = (char *) switch_xml_attr_soft(param, "value");
					}
				}
			}
			...
			if ( mod_smpp_gateway_create(&new_gateway, name, host, port, debug, system_id, password, system_type, profile) == SWITCH_STATUS_SUCCESS) {
				...
				if ( mon_ts != NULL ) { 
				    new_gateway->mon_ts = atoi(mon_ts);
				} else {
				    new_gateway->mon_ts = 15;
				}
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "monitoring_interval: %d\n",
				new_gateway->mon_ts);
				
				if ( data_coding_recognize != NULL ) { 
				    new_gateway->data_coding_recognize = atoi(data_coding_recognize);
				} else {
				    new_gateway->data_coding_recognize = 0;
				}
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "data_coding_recognize: %d\n",
				new_gateway->data_coding_recognize);
				
				switch_uuid_str(new_gateway->sip_smpp_gw_uuid, sizeof(new_gateway->sip_smpp_gw_uuid));
			} else {
	...
	mod_smpp_globals.pgsql = switch_pgsql_handle_new("hostaddr=127.0.0.1 dbname=sip_smpp_gw user=global password='_cfg.access'");
	if(mod_smpp_globals.pgsql == NULL) {
	    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "pgsql is NULL\n");
	} else {
	    if(switch_pgsql_handle_connect(mod_smpp_globals.pgsql) == SWITCH_PGSQL_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "pgsql is ready\n");
	    }
	}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_smpp_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	switch_mutex_init(&mod_smpp_globals.db_mutex, SWITCH_MUTEX_NESTED, mod_smpp_globals.pool);
	...
	SWITCH_ADD_API(mod_smpp_api_interface, "smpp_query", "...", mod_smpp_query_api, NULL);
	
	SWITCH_ADD_CHAT_APP(mod_smpp_chat_app_interface, "smpp_multi", "send multi message to gateway", "send multi message to gateway", 
						mod_smpp_chat_send_multi, "", SCAF_NONE);
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_smpp_shutdown)
{
	...
	if(mod_smpp_globals.pgsql) {
	    switch_pgsql_handle_disconnect(mod_smpp_globals.pgsql);
	}
	
	switch_mutex_destroy(mod_smpp_globals.db_mutex);
